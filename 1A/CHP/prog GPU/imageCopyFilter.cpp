...
/* print information on openCL version */
  ...
  options += " -D width=";
  options += std::to_string(image_width);
  options += " -D height=";
  options += std::to_string(image_height);
  //string options("");
  ...
  float sigma = 10.0;
  float beta = 1.0/(2.0*sigma*sigma);
  
  float *tab_coef;
  tab_coef = (float*)malloc((filter_size+1) * sizeof(float));
  
  for (int i = 0; i <= filter_size; i++){
    tab_coef[i] = exp(-(float) ((i*i*beta)));
  }
  ...
  cl_mem cl_output_image = clCreateBuffer(context, CL_MEM_WRITE_ONLY| CL_MEM_COPY_HOST_PTR,
				   image_width*image_height*channels * sizeof(unsigned char),
				   buffer, NULL);
  cl_mem cl_tab_coef = clCreateBuffer(context, CL_MEM_READ_ONLY| CL_MEM_COPY_HOST_PTR,
				      (filter_size+1) * sizeof(float),
				   tab_coef, NULL);
  ...
  status = clReleaseMemObject(cl_tab_coef);		//Release mem object.
