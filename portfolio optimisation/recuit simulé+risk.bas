...
Function Rendement(ByRef Solution() As Double, ByRef Coef() As Double)
    ...
    Rendement = Application.WorksheetFunction.SumProduct(Solution, Coef)  ' thats mu cher
...
Sub GraphiqueParetoCouleur1()
    ...
    End With
End Sub
Function Objectif(ByVal Rendement As Double, ByVal Risque As Double, ByVal PoidsRendement As Double, ByVal PoidsRisque As Double)
    Objectif = PoidsRendement * Rendement - PoidsRisque * Risque
End Function

Sub SimulatedAnnealing_risk()

    ' Déclaration des matrices et variables
    ...
    Dim Poids_Rendement As Double
    Dim Poids_Risque As Double
    ...
    ' Demande des poids à l'utilisateur
    On Error GoTo InvalidInput
    Poids_Rendement = InputBox("Veuillez entrer le poids du le rendement (poids_Rendement) :", "Saisie du Poids", 0.8)
    Poids_Risque = InputBox("Veuillez entrer le poids du le risque (poids_Risque) :", "Saisie du Poids", 0.2)
    If Poids_Rendement + Poids_Risque <> 1 Then
        MsgBox "Les poids doivent totaliser 1. Merci de relancer la procédure.", vbExclamation, "Erreur de Poids"
        Exit Sub
    End If
    On Error GoTo 0
   
    ' Initialisation du temps et aléatoire
    depart = Now
    ...
    ' Calcul initial du rendement, risque, et objectif pondéré
    Dim RendActuelle As Double, RisqueActuelle As Double
    Dim RendNouvelle As Double, RisqueNouvelle As Double
    RendActuelle = Rendement(Actuelle, Taux)
    RisqueActuelle = Risque(variance_solution(Matrice_cov, Actuelle))
    BestVal = Objectif(RendActuelle, RisqueActuelle, Poids_Rendement, Poids_Risque)
    ...
    For Iteration = 1 To NbIteration
        ...
        ' Calcul du rendement, risque, et objectif pondéré pour la nouvelle solution
        RendNouvelle = Rendement(Nouvelle, Taux)
        RisqueNouvelle = Risque(variance_solution(Matrice_cov, Nouvelle))
        Dim ValNouvelle As Double
        ValNouvelle = Objectif(RendNouvelle, RisqueNouvelle, Poids_Rendement, Poids_Risque)
       
        ' Mise à jour de la meilleure solution connue
        If ValNouvelle > BestVal Then
            For j = 1 To NbActions
        ...
        ' Mise à jour de la solution courante selon le critère d'acceptation
        If (ValNouvelle >= Objectif(RendActuelle, RisqueActuelle, Poids_Rendement, Poids_Risque)) Or _
           (Rnd <= Exp((ValNouvelle - Objectif(RendActuelle, RisqueActuelle, Poids_Rendement, Poids_Risque)) / Temperature)) Then
            For j = 1 To NbActions
                Actuelle(j) = Nouvelle(j)
            Next j
            RendActuelle = RendNouvelle
            RisqueActuelle = RisqueNouvelle
        End If
    ...
    Call AfficheSolution("Solution finale ", BestSol, Taux, BestVal, "", -1, Matrice_cov)
    arrivee = Now
    Call SolutionProposee(BestVal, BestSol, Int((arrivee - depart) * 3600 * 24))

    ' Calcul du front de Pareto pour les solutions avec risque/rendement
    Call Pareto(NbIteration)

    ' Affichage du graphique Pareto (risque vs rendement)
...
InvalidInput:
    MsgBox "Erreur dans la saisie des poids. Veuillez entrer des nombres valides.", vbCritical, "Erreur de Saisie"
    Exit Sub
