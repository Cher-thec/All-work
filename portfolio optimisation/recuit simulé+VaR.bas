...
End Sub
Function VaR(Rendement As Double, Risque As Double, Quantile As Double)
 
    VaR = Rendement - Risque * Application.NormSInv(Quantile * 0.01)
End Function
Sub AjouterValeursPareto()
    ...
    For i = 2 To DerniereLigne ' Supposons que les titres sont sur la première ligne
        ...
        If Pareto <> "Non Pareto" And Pareto <> "" Then
            ...
            Var99 = VaR(Rendement, Risque, 0.99) ' Appel de la fonction pour le quantile 99%
            Var95 = VaR(Rendement, Risque, 0.95) ' Appel de la fonction pour le quantile 95%
            Var80 = VaR(Rendement, Risque, 0.8)  ' Appel de la fonction pour le quantile 99%
            Var70 = VaR(Rendement, Risque, 0.7)  ' Appel de la fonction pour le quantile 95%
...
Sub SimulatedAnnealing_VaR()
    ...
    ' Constantes pour la Value at Risk
    Dim SeuilVaR As Double
    ...
    Dim ActuelleVaR As Double
    Dim NouvelleVaR As Double
    Dim BestVaR As Double
    Dim depart
    Dim arrivee
    ...
    ' Demander au client ses préférences de VaR
    SeuilVaR = InputBox("Entrez la VaR maximale acceptable :", "Seuil VaR")
    Quantile = InputBox("Entrez le quantile souhaité (95 ou 99) :", "Quantile VaR")
    ...
    BestVaR = VaR(BestVal, Risque(variance_solution(Matrice_cov, Actuelle)), Quantile)
    ...
    For Iteration = 1 To NbIteration
        ...
        ' Calcul de la VaR pour les nouvelles solutions
        NouvelleVaR = VaR(Rendement(Nouvelle, Taux), Risque(variance_solution(Matrice_cov, Actuelle)), Quantile)
        ActuelleVaR = VaR(Rendement(Actuelle, Taux), Risque(variance_solution(Matrice_cov, Actuelle)), Quantile)
        
        ' Mise à jour de la meilleure solution connue si elle respecte la VaR
        If NouvelleVaR <= SeuilVaR And Rendement(Nouvelle, Taux) > BestVal Then
            For j = 1 To NbActions
        ...
        If (NouvelleVaR <= SeuilVaR And Rendement(Nouvelle, Taux) >= Rendement(Actuelle, Taux)) Or _
           (Rnd <= Exp((Rendement(Nouvelle, Taux) - Rendement(Actuelle, Taux)) / Temperature) And NouvelleVaR <= SeuilVaR) Then
            For j = 1 To NbActions
